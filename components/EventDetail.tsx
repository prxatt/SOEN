...
const InsightWidgetContainer: React.FC<{title: string, icon: React.ReactNode, onAddToNotes?: () => void, onChat?: () => void, children: React.ReactNode}> = ({ title, icon, onAddToNotes, onChat, children }) => (
    <motion.div variants={itemVariants} className="relative bg-bg/50 p-3 rounded-xl border border-border">
         <div className="flex justify-between items-center mb-2">
            <h5 className="font-semibold text-xs flex items-center gap-2 text-text-secondary uppercase tracking-wider">{icon}{title}</h5>
...
const KeyMetricWidgetComponent: React.FC<{widget: KeyMetricWidget, onAddToNotes: () => void, onChat: () => void}> = ({widget, onAddToNotes, onChat}) => {
    ...
    <InsightWidgetContainer title={widget.title} icon={<Icon className={`w-4 h-4 ${widget.color}`}/>} onAddToNotes={onAddToNotes} onChat={onChat}>
        <div className="flex items-baseline gap-2">
            ...
            <p className="font-semibold text-text-secondary">{widget.unit}</p>
        </div>
...
const ChartWidgetComponent: React.FC<{widget: ChartWidget, onAddToNotes: () => void, onChat: () => void}> = ({widget, onAddToNotes, onChat}) => (
    <InsightWidgetContainer title={widget.title} icon={<Icons.ChartBarIcon className="w-4 h-4"/>} onAddToNotes={onAddToNotes} onChat={onChat}>
        <div className="h-40 w-full">
         ...
         <p className="text-xs text-text-secondary mt-1">{widget.commentary}</p>
    </InsightWidgetContainer>
);
     ...
     <InsightWidgetContainer title={widget.title} icon={<Icons.BoltIcon className="w-4 h-4"/>} onAddToNotes={onAddToNotes} onChat={onChat}>
        <div className="h-40 w-full relative">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
                ...
                <p className="text-sm font-semibold text-text-secondary">{widget.label}</p>
            </div>
        </div>
    </InsightWidgetContainer>
)
        ...
        <InsightWidgetContainer title={widget.title} icon={<Icon className="w-4 h-4 text-blue-400"/>} onAddToNotes={onAddToNotes} onChat={onChat}>
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="font-semibold text-text-secondary -mt-1">{widget.location}</p>
                </div>
                    ...
                    {widget.hourlyForecast.map((h, i) => {
                         ...
                         return (
                            <div key={i} className="flex flex-col items-center">
                                <span className="text-xs font-semibold text-text-secondary">{h.time}</span>
...
const GeneratedImageWidgetComponent: React.FC<{widget: GeneratedImageWidget, onAddToNotes: () => void, onChat: () => void}> = ({widget, onAddToNotes, onChat}) => {
    ...
    return (
        <InsightWidgetContainer title={widget.title} icon={<SparklesIcon className="w-4 h-4 text-accent"/>} onAddToNotes={onAddToNotes} onChat={onChat}>
            ...
            <p className="text-xs text-text-secondary mt-1 italic">"{widget.prompt}"</p>
        </InsightWidgetContainer>
    );
};
...
const EventDetail: React.FC<EventDetailProps> = ({ task, allTasks, notes, notebooks, projects, goals, updateTask, onComplete, onClose, redirectToKikoAIWithChat, addNote, categories, categoryColors, onAddNewCategory, triggerInsightGeneration, onViewNote, deleteTask }) => {
    ...
    const debouncedParse = useCallback(debounce(async (currentTask: Task) => {
        ...
        setIsParsing(true);
        const { data: parsedDetails } = await kikoRequest('parse_task_update', { command });

        if (parsedDetails && Object.keys(parsedDetails).length > 0) {
            const preservedTitle = command.substring(0, command.indexOf('/')).trim();
            const updated: Partial<Task> = { ...parsedDetails, title: preservedTitle };
            
            // Handle date/time separately
            if (parsedDetails.startTime instanceof Date) {
                const newStartTime = new Date(currentTask.startTime);
                newStartTime.setHours(parsedDetails.startTime.getHours(), parsedDetails.startTime.getMinutes());
                updated.startTime = newStartTime;
            }

            setEditableTask(prev => ({...prev, ...updated}));

            const dirty = new Set(dirtyFields);
            Object.keys(parsedDetails).forEach(key => dirty.add(key as keyof Task));
    ...
    const getFieldStyle = (field: keyof Task) => {
        ...
        return isDirty ? { 'boxShadow': `0 0 0 2px ${categoryColor}` } : {};
    };
                    ...
                    <div className="flex justify-between items-start">
                         <div className={isCompletedView ? 'text-white' : ''}>
                            ...
                            <div className="relative">
                                <input type="text" value={isCompletedView ? task.completionSummary?.newTitle || editableTask.title : editableTask.title} onChange={(e) => handleFieldChange('title', e.target.value)} className={`text-2xl sm:text-3xl font-bold font-display mt-1 bg-transparent w-full focus:outline-none rounded-sm transition-shadow`} style={getFieldStyle('title')} disabled={isCompletedView}/>
                ...
                <main className="p-4 sm:p-6 overflow-y-auto bg-bg/80">
                    {isCompletedView ? (
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 text-white">
                            <div className="lg:col-span-1 space-y-4 prose prose-sm prose-invert max-w-none">
                            ...
                            <div className="lg:col-span-2 space-y-4">
                                ...
                                <button onClick={handleRegenerateInsights} disabled={task.isGeneratingInsights} title="Regenerate Insights with latest health data" className="w-full text-sm p-2 rounded-lg bg-accent/10 hover:bg-accent/20 flex items-center justify-center gap-2">
                                ...
                                ) : <p className="text-sm text-text-secondary">No AI insight available for this completed task.</p>}
                            </div>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                            ...
                            <div className="lg:col-span-2 space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4"><div><label className="text-xs font-semibold text-text-secondary">Start Time</label><input type="time" value={new Date(editableTask.startTime).toTimeString().substring(0,5)} onChange={e => { const [h, m] = e.target.value.split(':'); const d = new Date(editableTask.startTime); d.setHours(parseInt(h), parseInt(m)); handleFieldChange('startTime', d); }} className={`w-full p-2 bg-bg border border-border rounded-lg mt-1 transition-shadow`} style={getFieldStyle('startTime')} /></div><div><label className="text-xs font-semibold text-text-secondary">Duration (min)</label><input type="number" value={editableTask.plannedDuration} onChange={e => handleFieldChange('plannedDuration', parseInt(e.target.value) || 0)} className={`w-full p-2 bg-bg border border-border rounded-lg mt-1 transition-shadow`} style={getFieldStyle('plannedDuration')} /></div></div>
                                <div>
                                    <label className="text-xs font-semibold text-text-secondary">Repeat</label>
                                    <select value={editableTask.repeat || 'none'} onChange={e => handleFieldChange('repeat', e.target.value as Task['repeat'])} className={`w-full p-2 bg-bg border border-border rounded-lg mt-1 transition-shadow`} style={getFieldStyle('repeat')}>
                                ...
                                <div className="space-y-3">
                                     <div><label className="text-xs font-semibold text-text-secondary flex items-center gap-1.5 mb-1"><Icons.BriefcaseIcon className="w-4 h-4"/> Project</label><select value={editableTask.projectId || ""} onChange={(e) => handleFieldChange('projectId', parseInt(e.target.value) || undefined)} className={`w-full p-2 bg-bg border border-border rounded-lg transition-shadow`} style={getFieldStyle('projectId')}><option value="">None</option>{projects.map(p => <option key={p.id} value={p.id}>{p.title}</option>)}</select></div>
                                      <div><label className="text-xs font-semibold text-text-secondary flex items-center gap-1.5 mb-1"><DocumentTextIcon className="w-4 h-4"/> Linked Note</label>
                                        {editableTask.notebookId && onViewNote ? (
                                             <button onClick={() => onViewNote(editableTask.notebookId!)} className={`w-full text-left p-2 bg-bg border border-border rounded-lg hover:bg-accent/10 truncate transition-shadow`} style={getFieldStyle('notebookId')}>{notes.find(n=>n.id === editableTask.notebookId)?.title || 'Select a Note'}</button>
                                        ) : (
                                            <select value={editableTask.notebookId || ""} onChange={(e) => handleFieldChange('notebookId', parseInt(e.target.value) || undefined)} className={`w-full p-2 bg-bg border border-border rounded-lg transition-shadow`} style={getFieldStyle('notebookId')}><option value="">None</option>{notes.map(note => <option key={note.id} value={note.id}>{note.title}</option>)}</select>
                                        )}
                                     </div>
                                     <div>
                                        <label className="text-xs font-semibold text-text-secondary flex items-center gap-1.5 mb-1"><LinkIcon className="w-4 h-4"/> Reference URL</label>
                                        {editableTask.referenceUrl ? <a href={editableTask.referenceUrl} target="_blank" rel="noopener noreferrer" className={`text-sm text-accent hover:underline block truncate p-2 bg-bg border border-border rounded-lg transition-shadow`} style={getFieldStyle('referenceUrl')}>{editableTask.referenceUrl}</a> : <input type="text" placeholder="https://coursera.org/..." value={editableTask.referenceUrl || ''} onChange={(e) => handleFieldChange('referenceUrl', e.target.value)} className={`w-full p-2 bg-bg border border-border rounded-lg transition-shadow`} style={getFieldStyle('referenceUrl')}/>}
                                    </div>
                                     <div className="relative">
                                        <label className="text-xs font-semibold text-text-secondary flex items-center gap-1.5 mb-1">
                                            <input type="checkbox" checked={editableTask.isVirtual} onChange={e => { const isChecked = e.target.checked; const updated = {...editableTask, isVirtual: isChecked}; if(isChecked){updated.location = ''} else {updated.linkedUrl = ''}; setEditableTask(updated); setDirtyFields(prev => new Set(prev).add('isVirtual')); }} className={`h-4 w-4 rounded border-gray-300 text-accent focus:ring-accent`} />
                                            <span>Virtual Event</span>
                                        </label>
                                        <div className="relative">
                                             <span className="absolute left-3 top-1/2 -translate-y-1/2 text-text-secondary">{editableTask.isVirtual ? <VideoCameraIcon className="w-4 h-4"/> : <MapPinIcon className="w-4 h-4"/>}</span>
                                             <input type="text" placeholder={editableTask.isVirtual ? "Zoom Link..." : "123 Main St, San Francisco, CA"} value={editableTask.isVirtual ? editableTask.linkedUrl || '' : editableTask.location || ''} onChange={(e) => editableTask.isVirtual ? handleFieldChange('linkedUrl', e.target.value) : handleLocationInputChange(e.target.value)} className={`w-full p-2 pl-9 bg-bg border border-border rounded-lg transition-shadow`} style={dirtyFields.has('location') || dirtyFields.has('linkedUrl') ? getFieldStyle('location') : {}} />
                                        </div>
                    ...
                    <div>
                        {!isCompletedView && (
                            <button
                                ...
                                className="text-sm font-semibold text-red-500 hover:bg-red-500/10 px-4 py-2 rounded-lg"
                            >
                                Delete Task
                            </button>
                        )}
